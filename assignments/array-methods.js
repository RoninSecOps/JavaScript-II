...
runners.map(function(arr) {
  firstNamesAllCaps.push(arr.first_name.toUpperCase());
});
...
runners.filter(function(arr) {
  if (arr.shirt_size === 'L') {
    runnersLargeSizeShirt.push(arr);
  }
});
...
let ticketPriceTotal = runners.reduce(function(acc, itemDonation) {
  return acc + itemDonation.donation;
}, 0);
...
const fruits = ['Apple', 'Orange', 'Plum', 'Kiwi', 'Cherry', 'Banana'];
// Problem 1
const fruitsLower = fruits.map(function(fruits) {
  return fruits.toLocaleLowerCase();
});
console.log(fruitsLower);
// Problem 2
const newArr = [];
const howManyLetters = fruits.forEach(function(fruit) {
  newArr.push(fruit.length);
});
console.log(newArr);
// Problem 3
